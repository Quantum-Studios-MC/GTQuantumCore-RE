...
public class SteamConstructionChamber extends GTQuantumCoreRecipeMapSteamMultiblockController {

	public SteamConstructionChamber(ResourceLocation metaTileEntityId) {
		super(metaTileEntityId, RecipeMapsHandler.CONSTRUCTION_CHAMBER, CONVERSION_RATE);
	...
	public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
		return new SteamConstructionChamber(metaTileEntityId);
	}

	@Override
	protected BlockPattern createStructurePattern() {
		return FactoryBlockPattern.start()
				.aisle("FFFFF", "FFFFF", "FFFFF", "FFFFF")
				.aisle("FFFFF", "F###F", "F###F", "FFFFF")
				.aisle("FFSFF", "FFFFF", "FFFFF", "FFFFF")
				.where('S', selfPredicate())
				.where('F', states(getCasingState()).setMinGlobalLimited(47).or(autoAbilities())
